...
export const createOrder = async (req, res, next) => {
  try {
    if (req.body.gigId) {
      ...
      res.status(400).send("Gig id debe ser obligatorio.");
    }
  } catch (err) {
    console.log(err);
...
export const getBuyerOrders = async (req, res, next) => {
  try {
    ...
    return res.status(400).send("Gig id debe ser obligatorio.");
  } catch (err) {
    console.log(err);
    return res.status(500).send("Error en el servicio interno.");
  }
...
export const getSellerOrders = async (req, res, next) => {
  try {
    ...
    return res.status(400).send("User id debe ser obligatorio");
  } catch (err) {
    console.log(err);
    return res.status(500).send("Error en el servicio interno");
  }
